...
\node[box, minimum height=1.2cm] (browser) at (0,0) {Browser\\(Client)};
\node[box, minimum height=1.2cm] (server) at (9,0) {Web\\Server};

\draw[->, thick] ([yshift=20pt]browser.east) -- node[above, font=\small\ttfamily] {1. GET / HTTP/1.1} ([yshift=20pt]server.west);
\draw[<-, thick] (browser.east) -- node[above, font=\small\ttfamily] {2. HTML (form)} (server.west);
\draw[->, thick] ([yshift=-20pt]browser.east) -- node[above, font=\small\ttfamily] {3. GET /echo?msg=Hello HTTP/1.1} ([yshift=-20pt]server.west);
\end{tikzpicture}
\end{center}
